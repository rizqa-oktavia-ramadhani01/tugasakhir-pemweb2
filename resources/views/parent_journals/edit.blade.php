                ...
                <div class="card border-0 shadow-sm rounded-4">
                    <div class="card-body p-4">
                        <h3 class="fw-bold mb-4">
                            ...
                            Edit Jurnal - {{ \Carbon\Carbon::parse($journal->tanggal)->translatedFormat('l, d F Y') }}
                        </h3>
                        <form action="{{ route('parent-journals.update', $journal->id) }}" method="POST">
                            ...
                            <div class="mb-3">
                                <label class="form-label fw-bold">Aktivitas yang Dilakukan <span
                                        class="text-danger">*</span></label>
                                <textarea name="aktivitas_dilakukan" class="form-control" rows="4"
                                    required>{{ $journal->aktivitas_dilakukan }}</textarea>
                            </div>

                            <div class="mb-3">
                                <label class="form-label fw-bold">Respon Anak</label>
                                <textarea name="respon_anak" class="form-control"
                                    rows="3">{{ $journal->respon_anak }}</textarea>
                            </div>
